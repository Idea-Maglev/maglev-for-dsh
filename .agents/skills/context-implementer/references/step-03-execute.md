...
当有依赖图排序结果（`{task_execution_order}`）时：
- 按拓扑排序确定的顺序执行任务
- 可并行任务仍串行执行，但按正确顺序（避免依赖倒置）
- 每完成一个任务，将 03_plan.md 中对应 checkbox 标记为 `[x]`
...
- 勾选任务： `- [x] Task N`（修改 03_plan.md 中的 checkbox）
- 只改 checkbox 状态，不改任务描述和 AC 引用
- 不删除已完成的任务行
- 不修改依赖图
- 立即继续下一个任务
